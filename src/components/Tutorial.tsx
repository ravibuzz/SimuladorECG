...
import { BookOpen, ArrowLeft, ArrowRight, Activity, Heart, Zap, Grid, Ruler, Layers, Search, AlertTriangle } from 'lucide-react';
...
const Tutorial: React.FC<TutorialProps> = ({ onComplete }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 font-sans text-slate-100">
      <div className="max-w-5xl w-full bg-slate-900 rounded-3xl shadow-2xl overflow-hidden border border-slate-800 flex flex-col min-h-[600px]">
        {/* Header */}
        <div className="bg-slate-800/50 p-6 border-b border-slate-700 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Passo {step + 1}</p>
              <h2 className="text-xl font-black text-white">{current.title}</h2>
            </div>
          </div>
          <div className="text-right hidden sm:block">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Progresso</p>
            <p className="text-lg font-black text-emerald-400">{step + 1} / {steps.length}</p>
          </div>
        </div>
          ...
          <div className="flex-1 space-y-8">
            ...
            <div className="grid grid-cols-1 gap-4">
              {current.details.map((det, idx) => (
                <div key={idx} className="bg-slate-800 border border-slate-700 rounded-2xl p-5 flex items-center justify-between group hover:border-emerald-500/50 transition-colors">
                  <span className="text-emerald-400 font-bold text-xs uppercase tracking-widest">{det.label}</span>
                  <span className="text-white font-bold text-lg">{det.value}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Footer */}
        <div className="p-6 bg-slate-800/30 border-t border-slate-700 flex flex-col gap-4 relative">
          <div className="flex justify-between items-center w-full">
            <button 
              onClick={() => setStep(Math.max(0, step - 1))} 
              disabled={step === 0}
              className="flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-slate-400 hover:text-white hover:bg-slate-800 disabled:opacity-0 transition-all"
            >
            ...
            <div className="flex gap-1.5">
              {steps.map((_, i) => (
                <div 
                  key={i} 
                  className={`h-1.5 rounded-full transition-all duration-300 ${i === step ? 'bg-emerald-400 w-6' : 'bg-slate-700 w-1.5'}`}
                />
              ))}
            </div>
            ...
            {step < steps.length - 1 ? (
              <div className="flex gap-4 items-center">
                ...
                <button 
                  ...
                  className="flex items-center gap-2 px-8 py-3 bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-black rounded-xl shadow-lg shadow-emerald-500/20 transition-all active:scale-95"
                >
              ...
              <button 
                onClick={onComplete}
                className="flex items-center gap-2 px-8 py-3 bg-indigo-500 hover:bg-indigo-400 text-white font-black rounded-xl shadow-lg shadow-indigo-500/20 transition-all animate-pulse active:scale-95"
              >
                Começar Prática <Activity className="w-5 h-5" />
              </button>
            )}
          </div>
