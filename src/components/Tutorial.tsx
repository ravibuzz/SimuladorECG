...
const Tutorial: React.FC<TutorialProps> = ({ onComplete }) => {
  ...
  const steps = [
    ...
    },
    {
      ...
      text: "As 12 derivações funcionam como câmeras posicionadas em diferentes pontos ao redor do coração, permitindo ver a eletricidade em 2D nos planos correspodentes.",
      highlight: "all",
    ...
    },
    {
      ...
      text: "A distância entre dois picos da onda R na mesma derivação determina a frequência cardíaca e a regularidade do ritmo.",
      highlight: "rr",
    ...
    },
    {
      ...
      details: [
        ...
        { label: "Patologia", value: "> 200 ms indica Bloqueio AV de 1° grau" }
      ]
    },
    {
    ...
    },
    {
      ...
      text: "O Ponto J é onde o QRS termina e o Segmento ST começa. O Segmento ST deve ser plano (isoelétrico). Desníveis aqui indicam isquemia ou infarto.",
      highlight: "st",
      icon: <Activity className="w-6 h-6 text-rose-400" />,
      details: [
        { label: "Ponto J", value: "Junção final QRS-segmento ST" },
    ...
    },
    {
      ...
      text: "Mede todo o tempo de atividade ventricular (despolarização + repolarização). Varia de acordo com a frequência cardíaca. Alterações no intervalo QT predispõem a arritmias.",
      highlight: "qt",
    ...
    },
    {
      ...
      details: [
        ...
        { label: "Condução", value: "Uma onda P para cada QRS" }
      ]
    },
    {
  ...
  const getStroke = (part: string) => {
    ...
    if (current.highlight === part) {
      ...
      if (part==='tp' || part==='rr' || part==='calpulse' || part==='isodiphasic') return '#10b981';
    }
    return '#475569'; 
  };
      ...
      <div className="max-w-5xl w-full bg-slate-900 rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden border border-slate-800 flex flex-col min-h-[500px] sm:min-h-[600px]">
        ...
        <div className="bg-slate-800/50 p-4 sm:p-6 border-b border-slate-700 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <button 
                onClick={onComplete}
                className="text-slate-400 font-bold hover:text-white transition-colors text-xs sm:text-sm"
              >
                Pular Tutorial
            </button>
        ...
        <div className="flex-1 p-6 sm:p-8 md:p-12 flex flex-col md:flex-row gap-6 sm:gap-12 items-center overflow-y-auto">
          ...
          <div className={`w-full md:w-80 aspect-[4/3] rounded-2xl sm:rounded-3xl border flex items-center justify-center relative overflow-hidden transition-all duration-700 shrink-0 ${current.highlight === 'grid' ? 'scale-105 border-red-400' : 'border-slate-700'}`} style={{ backgroundColor: current.highlight === 'grid' ? '#f87171' : '#1e293b', backgroundImage: current.highlight === 'grid' ? `linear-gradient(rgba(255,100,100,0.4) 1px, transparent 1px), linear-gradient(90deg, rgba(255,100,100,0.4) 1px, transparent 1px), linear-gradient(rgba(255,50,50,0.7) 1px, transparent 1px), linear-gradient(90deg, rgba(255,50,50,0.7) 1px, transparent 1px)` : 'none', backgroundSize: '15px 15px, 15px 15px, 75px 75px, 75px 75px', backgroundPosition: 'center' }}>
             <svg viewBox={current.highlight === 'rr' ? "0 -20 440 240" : "0 -20 340 240"} className="absolute inset-0 w-full h-full drop-shadow-md">
                 ...
                 {current.highlight === 'isodiphasic' ? (
                   <path d="M 105 100 L 115 100 L 125 40 L 135 160 L 145 100" fill="none" stroke="#818cf8" strokeWidth="4" strokeLinejoin="round" />
                 ) : (
                 ...
                 {current.highlight === 'rr' && ( 
                    <g className="animate-in fade-in">
                        <path d="M 320 100 L 330 100 C 340 85, 360 85, 370 100 L 385 100 L 395 115 L 405 30 L 415 125 L 425 100 L 440 100" fill="none" stroke="#475569" strokeWidth="4" strokeLinejoin="round" strokeLinecap="round" opacity="0.6" />
                        <path d="M 125 20 L 125 10 L 405 10 L 405 20" fill="none" stroke="#10b981" strokeWidth="2.5" />
                        <text x="265" y="32" fill="#34d399" fontSize="13" textAnchor="middle" fontWeight="bold">Intervalo RR</text>
                    </g> 
                 )}
